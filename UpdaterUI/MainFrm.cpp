...
{
	if (CanSendIPUpdates() && !IsLeftAltAndCtrlPressed()) {
		SwitchToHiddenState();
		SetMsgHandled(TRUE);
	} else {
		SetMsgHandled(FALSE);
...
void CMainFrame::OnExitButtonClicked(UINT /*uNotifyCode*/, int /*nID*/, CWindow /*wndCtl*/)
{
...
bool CMainFrame::DnsVsHttpIpMismatch()
{
	...
	ipFromDns.Format(_T("%u.%u.%u.%u"), (a >> 24) & 255, (a >> 16) & 255, (a >> 8) & 255, a & 255);
	TCHAR *ip = StrToTStr(m_ipFromHttp);
	int cmp = ipFromDns.Compare(ip);
	free(ip);
	if (0 == cmp)
		return false;
	return true;
...
void CMainFrame::BuildStatusEditRtf(RtfTextInfo& ti)
{
	...
	if (ipMismatch) {
		ti.AddTxt(_T("Your OpenDNS filtering settings might not work due to DNS and HTTP IP address mismatch."));
		ti.AddPara();
		ti.AddPara();
	}
...
void CMainFrame::DoLayout()
{
	...
	// position "Exit" button in the bottom right corner
	CUIButtonSizer exitSizer(m_buttonExit);
	static const int BTN_EXIT_RIGHT_MARGIN = 8;
	static const int BTN_EXIT_BOTTOM_MARGIN = 4;
	RECT pos;
	CalcFixedPositionBottomRight(clientDx, clientDy, BTN_EXIT_RIGHT_MARGIN, BTN_EXIT_BOTTOM_MARGIN, &exitSizer, pos);
	m_buttonExit.MoveWindow(&pos);
	...
	linkSizer.SetFont(m_linkAboutFont);
	s = linkSizer.GetIdealSize2();
	static const int LINK_ABOUT_BOTTOM_MARGIN = 4;
...
int CMainFrame::OnCreate(LPCREATESTRUCT /* lpCreateStruct */)
{
	...
	m_linkAboutFont = AtlGetDefaultGuiFont();
	...
	m_linkAbout.SetFont(m_linkAboutFont);
	m_linkAbout.SetDlgCtrlID(IDC_LINK_ABOUT);

	m_buttonExit.Create(m_hWnd, r, _T("Exit"), WS_CHILD | WS_VISIBLE);
	m_buttonExit.SetFont(m_defaultFont);
	m_buttonExit.SetDlgCtrlID(IDC_BUTTON_EXIT);
